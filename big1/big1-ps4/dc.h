...
struct ScriptFile
{
	...
	int64_t  				m_numEntries;							///< <c>0x10</c>: number of state-scripts/symbols declared in the file 
	ScriptFileEntry			m_aEntries[];							///< <c>0x18</c>: variable sized array of ScriptFileEntry entries
};

struct ScriptFileEntry //size = 0x20
{
	...
	Igc*					m_pIgc;    								///< <c>0x18</c>: pointer to the Igc
};
...
struct Igc
{
	...
	const char*			m_scriptName;								///< <c>0x08</c>: ptr to the script name 
	const char*			m_unkString;								///< <c>0x10</c>: no idea maybe a label ?
...
struct TrackName //size = 0x10
{
	...
	const char*			m_pTrackname;								///< <c>0x08</c>: ptr to the track name 
};
...
struct Lambda //size = 0x28
{
	...
	const char*			m_pBlockName;								///< <c>0x08</c>: ptr to the block name
